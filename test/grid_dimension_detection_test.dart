...
void main() {
  group('Automatic Grid Dimension (N x N) Detection Tests', () {
    test(
      'Automatically detects 9x9 board for IMG_4554 without specifying n',
      () {
        final image = img.decodeImage(
          File('test/fixtures/screenshots/IMG_4554.PNG').readAsBytesSync(),
        )!;
    ...
    test(
      'Automatically detects 10x10 board for IMG_4552 without specifying n',
      () {
        final image = img.decodeImage(
          File('test/fixtures/screenshots/IMG_4552.PNG').readAsBytesSync(),
        )!;
        final result = VisionPipeline.processImage(image); // n is omitted
        ...
        final solverResult = MeowdokuSolver(
          result.puzzleBoard,
        ).solve(initialFixedQueens: result.detectedFixedQueens);
        expect(solverResult.isSolved, isTrue);
    ...
    test(
      'Automatically detects 12x12 board for IMG_4557 without specifying n',
      () {
        final image = img.decodeImage(
          File('test/fixtures/screenshots/IMG_4557.PNG').readAsBytesSync(),
        )!;
        final result = VisionPipeline.processImage(image); // n is omitted
    ...
    test(
      'Automatically detects 10x10 board for IMG_4560 without specifying n',
      () {
        final image = img.decodeImage(
          File('test/fixtures/screenshots/IMG_4560.PNG').readAsBytesSync(),
        )!;
        final result = VisionPipeline.processImage(image); // n is omitted
