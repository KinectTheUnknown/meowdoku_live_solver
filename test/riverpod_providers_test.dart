...
void main() {
  group('Riverpod Providers Unit Tests', () {
    test(
      'SolverController state mutations work correctly and return updated state',
      () {
        ...
        final afterFunctional = notifier.update(
          (oldState) => oldState.copyWith(autoSolveIntervalSec: 3.0),
        );
        expect(afterFunctional.autoSolveIntervalSec, equals(3.0));
        expect(
          container.read(solverControllerProvider).autoSolveIntervalSec,
          equals(3.0),
        );
        ...
        // Test clearSolution
        final cleared = notifier.clearSolution(
          cacheHits: 0,
          cacheMisses: 0,
          cacheSize: 0,
        );
    ...
    test(
      'VideoPlayerController mutations update state and return updated state',
      () {
        final container = ProviderContainer();
        addTearDown(container.dispose);
        ...
        final custom = notifier.update(
          (oldState) => oldState.copyWith(videoWidth: 1280),
        );
        expect(custom.videoWidth, equals(1280));
        expect(
          container.read(videoPlayerControllerProvider).videoWidth,
          equals(1280),
        );
      },
    );
    ...
    test('VdoNinjaStream initialized and functional update works', () {
      ...
      final updated = notifier.update(
        (oldState) => oldState.copyWith(activeStreamId: 'stream-123'),
      );
      expect(updated.activeStreamId, equals('stream-123'));
      expect(
        container.read(vdoNinjaStreamProvider).activeStreamId,
        equals('stream-123'),
      );
    });
    ...
    test('AutoSolveService toggleAutoSolve updates state', () {
      ...
      expect(container.read(autoSolveServiceProvider), isFalse);
      expect(
        container.read(solverControllerProvider).autoSolveEnabled,
        isFalse,
      );
    });
  });
