...
class LabColor {
  ...
  factory fromRgb(int r, int g, int b) {
    ...
    inR = (inR > 0.04045)
        ? math.pow((inR + 0.055) / 1.055, 2.4).toDouble()
        : (inR / 12.92);
    inG = (inG > 0.04045)
        ? math.pow((inG + 0.055) / 1.055, 2.4).toDouble()
        : (inG / 12.92);
    inB = (inB > 0.04045)
        ? math.pow((inB + 0.055) / 1.055, 2.4).toDouble()
        : (inB / 12.92);
  ...
  @override
  String toString() =>
      'LabColor(L: ${l.toStringAsFixed(1)}, a: ${a.toStringAsFixed(1)}, b: ${b.toStringAsFixed(1)})';
}
